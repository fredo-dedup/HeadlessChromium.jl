...
@time tg2 = Target("file://C:/temp/VegaLite plot.html")
...
send(tg2, "Page.captureScreenshot", format="png") do resp
    open("c:/temp/sandbox3.png", "w") do io
        write(io, base64decode(resp["result"]["data"]))
    end
end

send(tg2, "DOM.performSearch", query=".marks") do resp
    send(tg2, "DOM.getBoxModel", nodeId=".marks") do resp
    println("search id : $(resp["result"]["searchId"]), count : $(resp["result"]["resultCount"])")
end


send(tg2, "DOM.performSearch", query=".marks")
send(tg2, "DOM.getSearchResults", searchId="9552.14", fromIndex=0, toIndex=1)

send(tg2, "DOM.getBoxModel", nodeId=46)
...
tstd = Dict{String,Any}(Pair{String,Any}("method", "DOM.setChildNodes"),Pair{String,Any}("params", Dict{String,Any}(Pair{String,Any}("parentId", 0),Pair{String,Any}("nodes", Any[Dict{String,Any}(Pair{String,Any}("nodeId", 3),Pair{String,Any}("nodeName", "#document"),Pair{String,Any}("localName", ""),Pair{String,Any}("nodeValue", ""),Pair{String,Any}("childNodeCount", 1),Pair{String,Any}("nodeType", 9),Pair{String,Any}("baseURL",        "file:///C:/temp/VegaLite%20plot.html"),Pair{String,Any}("documentURL", "file:///C:/temp/VegaLite%20plot.html"),Pair{String,Any}("backendNodeId", 2),Pair{String,Any}("xmlVersion", ""))]))))
